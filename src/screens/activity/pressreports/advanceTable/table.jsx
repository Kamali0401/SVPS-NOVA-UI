...
export function TableComp({
  ...
  const filterData = (value) => {
    ...
    const dataIs = data.filter((item) => {
      const date = dayjs(JSON.parse(item.data).DateOfPublished).format("DD-MM-YYYY");
    ...
    <>
      ...
      >
        ...
        <tbody {...getTableBodyProps()}>
          {page.map((row, i) => {
            prepareRow(row);
            const date = dayjs(JSON.parse(row.values.data).DateOfPublished).format(
              "DD-MM-YYYY"
            );
            ...
            return (
              <tr {...row.getRowProps()}>
                <td>{row.index + 1}</td>
                {/*<td>{row.values.id}</td>*/}
                <td>{JSON.parse(row.values.data).NameOfNewspaper}</td>
                <td>{JSON.parse(row.values.data).PressReportsType}</td>
                <td>{`${dateString}`}</td>
                ...
                <td className="d-row">
                  ...
                  <span
                    onClick={() => {
                      if (window.confirm('Are you sure you want to delete \"'+ JSON.parse(row.values.data).NameOfNewspaper+ "\" newspaper\'s  record in Press Report?")) deleteActivityProp(row.values);      
                    }}
                    className="mx-2"
                  >
