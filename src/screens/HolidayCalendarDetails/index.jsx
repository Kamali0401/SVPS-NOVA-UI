...
import AddHolidayCalendarDetails from "./addHolidayCalendarDetails/index";
...
const HolidayCalendarDetails = () => {
  // set up dispatch
  const dispatch = useDispatch();
  // const navigate = useNavigate();
  const { loading, error, holidayCalendarList } = useSelector(holidayCalendarSelector);
  const [IsEdit, setIsEdit] = useState(false);
  const [showPopup, setshowPopup] = useState(false);
  const [assessment, setassessment] = useState("");
  const [holidayCalendar, setHolidayCalendar] = useState();
  const [listOfHolidayCalendar, setlistOfHolidayCalendar] = useState(holidayCalendarList || []);
  const [file, setFile] = useState();
  const [filesList, setfilesList] = useState([]);
  const [fileName, setFileName] = useState();
  const [selectedFile, setselectedFile] = useState("");
  const togglePopup = () => {
    onAssessmentSelection("HolidayCalendar Details");
  ...
  const onFileUpload = async (res) => {
    ...
    if(selectedFile[0].name == "HolidayCalendar.csv")
    {
      debugger;
      ...
      formData.append("TypeofUser", 'HolidayCalendar');
    ...
    try {
      ...
      alert('File Name should be HolidayCalendar.csv only')
    }
  ...
  // console.log(loading);
  console.log("Fetch data from Store", holidayCalendarList);

  const deleteHolidayCalendarFun = (item) => {
    
    dispatch(deleteHolidayCalendar(item.id));
  };

  const editHolidayCalendar = (item) => {
    setHolidayCalendar({ ...item, data: item });
    setshowPopup(!showPopup);
    setIsEdit(true);
  };
  ...
  const getHolidayCalendarData = () => {
   
    dispatch(fetchHolidayCalendars());
  };

  useEffect(() => {
    if (!holidayCalendarList) {
      return;
    }
    setlistOfHolidayCalendar(holidayCalendarList);
  }, [holidayCalendarList]);

  console.log("Before Render holidayCalendarList", holidayCalendarList);
  console.log("Before Render listOfHolidayCalendar", listOfHolidayCalendar);
    ...
    return (
      <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div className="formsec">
          <div className="row">
            <h2 className="heading col pt-2">Holiday Calendar </h2>
          ...
          <div className="bootstrap-table">
            ...
            <div className="fixed-table-container">
              ...
              <AdvanceTable
                  deleteHolidayCalendarProp={(item) => deleteHolidayCalendarFun(item)}
                  editHolidayCalendarProp={(item) => editHolidayCalendar(item)}
                  data={listOfHolidayCalendar}
                />
              </div>
            </div>
          </div>
